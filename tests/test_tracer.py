...
class TestTracer(unittest.TestCase):
    ...
    def test_printer_functionality(self):
        ...
        # Test the NoOpPrinter
        noop_printer = NoOpPrinter()
        noop_printer.print("This should do nothing")  # Should not raise exceptions

        # NoOpPrinter doesn't have set_verbosity, which is fine
    ...
    def test_formatter_functionality(self):
        ...
        tracer.enable(verbosity=0)

        # Test format_value method
        result = Tracer._format_value(12345)
        self.assertEqual(result, "12345")

        # Test with a string that would be truncated
        long_string = "x" * 200
        result = Tracer._format_value(long_string)
        ...
        obj = TestObj()
        result = Tracer._format_value(obj)
    ...
    def test_ai_config(self):
        ...
        self.assertFalse(tracer.ai_config.enabled)

        # Test enabling AI
        tracer.enable(
            enable_ai=True, api_key="test_key", model="test_model", base_url="https://test.com"
        )

        # Verify settings were applied
        self.assertTrue(tracer.ai_config.enabled)
        self.assertEqual(tracer.ai_config.api_key, "test_key")
        self.assertEqual(tracer.ai_config.model, "test_model")
        self.assertEqual(tracer.ai_config.base_url, "https://test.com")
